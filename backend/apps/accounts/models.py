...
class Portfolio(models.Model):
    """The Super Admin's personal portfolio, generated by AI from their resume.

    One portfolio per Super Admin. The admin uploads their own resume; the AI
    reviews it (pros/cons/improvements - private, only the admin sees it) and
    builds a public portfolio (headline, about, skills, education, experience,
    projects) from it. The AI can also rewrite the resume into a polished
    "rebuilt" version - delivered as an editable text preview plus a
    downloadable .docx - and review that rebuilt version too. The public page
    (/portfolio/<slug>) is reachable by anyone, logged in or not, but only
    when ``is_published`` is on. The admin's resume and its reviews are never
    shown to faculty or students - they are private to the Super Admin.
    """
    ...
    # Public link identity: /portfolio/<slug>. Regenerable by the admin.
    slug = models.CharField(max_length=80, unique=True, blank=True, default="")
    ...
    # Extra editable sections the admin adds: [{"title", "content"}] (ordered).
    custom_sections = models.JSONField(default=list, blank=True)

    # The admin's uploaded resume (Cloudinary raw file). Private to the admin.
    ...
    # AI review of the uploaded resume - private to the admin.
    ai_status = models.CharField(
        max_length=10, choices=AiStatus.choices, default=AiStatus.PENDING
    )
    ai_score = models.PositiveSmallIntegerField(null=True, blank=True)  # 0-100
    ...
    # AI-rebuilt resume: structured sections + plain text + downloadable .docx.
    rebuilt_sections = models.JSONField(null=True, blank=True)
    rebuilt_text = models.TextField(blank=True, default="")
    rebuilt_file_name = models.CharField(max_length=255, blank=True, default="")
    rebuilt_docx_url = models.URLField(max_length=500, blank=True, default="")
    rebuilt_docx_public_id = models.CharField(max_length=255, blank=True, default="")
    rebuilt_at = models.DateTimeField(null=True, blank=True)
    # Review of the rebuilt version ("the again review").
    rebuilt_ai_status = models.CharField(
        max_length=10, choices=AiStatus.choices, default=AiStatus.PENDING
    )
    rebuilt_ai_score = models.PositiveSmallIntegerField(null=True, blank=True)
    rebuilt_ai_analysis = models.JSONField(null=True, blank=True)
    rebuilt_ai_error = models.CharField(max_length=500, blank=True, default="")
    rebuilt_ai_analyzed_at = models.DateTimeField(null=True, blank=True)

    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self) -> str:
        return f"Portfolio({self.user.roll_number})"
